...
def api_scan():
    ...
    if not token_id: return jsonify({"status":"error","message":"token_id missing"}),400
    ...
    db.session.add(ScanLog(token_id=token_id, scan_type=scan_type, bundle_code=bundle_code or None, operation_code=operation_code or None))
    if scan_type=="work":
        op = Operation.query.filter_by(code=operation_code).first()
        db.session.add(ProductionLog(worker_id=w.id, operation_id=op.id if op else None, quantity=1, status="COMPLETED"))
    ...
    scans_today = ScanLog.query.filter(ScanLog.token_id==token_id, ScanLog.scan_type=="work", ScanLog.scanned_at>=start, ScanLog.scanned_at<=end).count()
    ...
    return jsonify({
        ...
        "earnings": round(earnings_inr/83.0,2) # as per your ESP32 display logic
    })
...
def api_stats():
    ...
    today = datetime.utcnow().date()
    start = datetime.combine(today, time.min); end = datetime.combine(today, time.max)
    scans_today = ScanLog.query.filter(ScanLog.scan_type=="work", ScanLog.scanned_at>=start, ScanLog.scanned_at<=end).count()
...
def api_activities():
    ...
    items = (db.session.query(ScanLog, Worker)
...
def api_workers():
    ...
    return jsonify([{
        "id": w.id, "worker_id": w.worker_id, "name": w.name,
        "department": w.department, "skill": w.skill, "rate": w.rate,
        "line": w.line, "token_id": w.token_id, "is_logged_in": w.is_logged_in,
...
def add_worker():
    ...
    w = Worker(name=name, department=department, token_id=token_id, rate=rate, worker_id=worker_id)
...
def api_operations():
    ...
    return jsonify([{"id":o.id,"code":o.code,"name":o.name,"description":o.description} for o in ops])
...
def add_operation():
    ...
    db.session.add(Operation(code=code, name=name, description=desc)); db.session.commit()
    return ("",204)

@app.get("/api/production")
def api_production():
    rows = (db.session.query(ProductionLog, Worker, Operation)
        ...
        .order_by(ProductionLog.timestamp.desc()).limit(200).all())
    return jsonify([{
        "id": p.id,
